class Pebble {
    static createCanvas(
        ...
        canvas.setAttribute("style", `width: ${canvas.width}px; height: ${canvas.height}px; border: ${border}; background: ${background};`);

        return canvas;
    }

    static createStage(width = 0, height = 0) {
        let thing = new DisplayObject();
        thing.width = width;
        thing.height = height;
        return thing;
    }
...
class SpriteCanvasObject extends DisplayObject {
    constructor(
        source,
        x = 0,
        y = 0
